...
measures.list<-c('<a href = "https://www.census.gov/programs-surveys/acs/about.html" target = "new" >The American Community Survey</a>',
                 ...
                 #'<a href = "https://appliedgeographic.com/crimerisk/" target = "new" >Applied Geographic Solutions CrimeRisk Data</a>', ##Inspace specific - not publically available
                 #'<a href = "https://journals.sagepub.com/doi/10.1177/0033354920968799" target = "new" >Sidewalk Score</a>', ## not publically available
...
ratings.table<-dataset %>%
  mutate('GeocodeRating'=ifelse(rating==0, '0', ifelse(rating>0 & rating<10, '0 to 9', ifelse(rating>9 & rating < 21, '10-20', ifelse(GeocodeRating>20, '20 or higher', NA)))))%>%
  group_by('GeocodeRating')%>%dplyr::summarise(n=n())%>%mutate(percent=round(n/sum(n), 2)) %>%
  dplyr::select('GeocodeRating', n, percent)
